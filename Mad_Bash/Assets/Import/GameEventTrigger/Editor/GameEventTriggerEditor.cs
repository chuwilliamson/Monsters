...
public class GameEventTriggerEditor : Editor
{
    ...
    /// <summary>
    ///   <para>
    ///   Implement specific EventTrigger inspector GUI code here. 
    ///   If you want to simply extend the existing editor call the base OnInspectorGUI () before doing any custom GUI code.</para>
    /// </summary>
    public override void OnInspectorGUI()
    {
        ...
        for (var index = 0; index < _mEntriesProperty.arraySize; ++index)
        {
            ...
            var propertyRelative2 = arrayElementAtIndex.FindPropertyRelative("Response");
            ...
            var propertyRelative5 = arrayElementAtIndex.FindPropertyRelative("RequiresSender");
            ...
            GUI.enabled = false;
            EditorGUILayout.PropertyField(propertyRelative1, _mEventIdName, new GUILayoutOption[0]);//event
            GUI.enabled = true;

            propertyRelative5.boolValue = EditorGUILayout.BeginToggleGroup("Requires Sender?", propertyRelative5.boolValue);
            if (propertyRelative5.boolValue)//sender
    ...
    private void ShowAddTriggermenu()
    {
        ...
        for (var index1 = 0; index1 < _mEventTypes.Length; ++index1)
        {
            ...
            {
                if (_mEntriesProperty.GetArrayElementAtIndex(index2).FindPropertyRelative("EventID").intValue == index1)
                    flag = false;
            }
    ...
    private void OnAddNewSelected(object index)
    {
        ...
        _mEntriesProperty.GetArrayElementAtIndex(_mEntriesProperty.arraySize - 1).FindPropertyRelative("EventID").intValue = num;
        _mEntriesProperty.GetArrayElementAtIndex(_mEntriesProperty.arraySize - 1).FindPropertyRelative("Name").stringValue = _names[num];
        _mEntriesProperty.GetArrayElementAtIndex(_mEntriesProperty.arraySize - 1).FindPropertyRelative("Event").objectReferenceValue = _mGameEventArgs[num];
        serializedObject.ApplyModifiedProperties();
    }
}
